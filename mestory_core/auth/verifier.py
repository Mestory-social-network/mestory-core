...
class AccessTokenVerifier:
    ...
    async def verify(self, token: str) -> AccessTokenClaims:
        ...
        потомки, а также `TypeError` из сломанного документа JWKS)
        нарочно не перехватываются здесь и уходят наружу как есть: это не
        «токен плохой», а «мы не можем проверить», и заворачивать их в
        `jwt.InvalidTokenError` означало бы неотличимо превратить аварию
        источника в 401 для всех пользователей сразу.
        ...
        :raises TypeError: если документ JWKS получен, но повреждён (ключ
            не является RSA).
        """
        try:
