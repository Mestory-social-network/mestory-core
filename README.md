...
```bash
uv add git+https://github.com/Mestory-social-network/mestory-core@v0.1.4
```
...
Строка установки выше, `version` в `pyproject.toml`, `__version__` в
`mestory_core/__init__.py` и версия собственного editable-пакета в
`uv.lock` обязаны совпадать с тегом, который на них указывает, — иначе
README инструктирует ставить версию, отличную от той, что описывает (а
`uv.lock`, разошедшийся с `pyproject.toml`, делает дерево, помеченное
тегом, внутренне противоречивым, пусть эта нестыковка на установке
потребителем и не сказывается). Поэтому выпуск версии — один коммит, а
не тег на случайный коммит:

1. В одном коммите обновить все четыре места сразу: строку установки в
   этом README, `version` в `pyproject.toml`, `__version__` в
   `mestory_core/__init__.py`, и `uv.lock` — выполнить `uv lock` (или
   любую другую команду `uv`, синхронизирующую лок-файл, например `uv
   sync`), чтобы подтянуть новую версию в собственную запись пакета.
